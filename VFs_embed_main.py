...
parser.add_argument('--encode', type=str, default='embed')   # onehot or word2vec or embed(1,2..)
...
model = None
model = embed_amps(embedding_vector_length, max_sequence_length, len(class_name))
model.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy'])

sig = "embed_seed" + str(args.signal)
train_info_record = data_dir + args.file + "_record/embed/"
...
history = model.fit_generator(generator=training_generator, validation_data=val_generator, epochs=args.epoch,
                                   verbose=2, callbacks=[checkpoint, reduce_lr], class_weight=class_weights)
